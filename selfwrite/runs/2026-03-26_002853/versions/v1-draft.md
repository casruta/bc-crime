...
No single province drove this pattern. When each province's 2004 crime rate is set to a baseline of 100, all trajectories slope downward at similar rates. Saskatchewan's crime rate remained roughly 2.8x Ontario's in absolute terms, but both fell at comparable speeds. That every province moved in the same direction at the same time points to structural causes (an ageing population, urbanization, changes in enforcement philosophy) rather than any one province's policies.
...
The divergence in individual violation trajectories has a practical consequence: the offences driving the aggregate decline (vehicle theft, break-ins) differ fundamentally from those driving the increase (child exploitation, shoplifting), and each requires a distinct intervention model.
