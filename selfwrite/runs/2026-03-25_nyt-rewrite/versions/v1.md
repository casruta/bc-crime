...
But the composition tells a different story. Since 2014, the violent component of the CSI has been climbing while the non-violent component falls. The overall index keeps dropping because property crime's decline is larger in absolute terms. Underneath, though, a crossover is underway: the crimes that scare people most are getting worse.
...
Figure 3 shows the year-over-year record: 18 years of decline against 8 of increase, with the most recent year posting a 7.4% drop.

![Figure 3: BC year-over-year crime rate changes -- 18 declining years vs. 8 increasing](outputs/charts/q1_bc_yoy_trend.png)

This isn't a BC-specific trend. Saskatchewan runs at roughly 2x Ontario's rate, but every province follows the same downward arc. Indexed to 2004 = 100, the lines converge. The decline is structural -- driven by demographics, urbanization, and enforcement philosophy -- not by any single province's policy.
...
BC still sits 21% above the national average (Figure 6), a gap that has persisted for the entire 20-year series. Its 7.4% year-over-year decline ranks second among provinces (Figure 7).
...
Figure 8 breaks down which offences contribute most to BC's overall CSI, revealing the violations that carry the most weight in the index and the most leverage for policy intervention.
...
One caveat: the CSI weights offences by average sentence severity. When a Criminal Code revision reclassifies an act to a more serious category, the index rises even if the underlying behaviour is unchanged. The post-2019 amendments to domestic violence and sexual assault definitions may account for some of the violent-crime increase.

## 2. Property Crime Is Collapsing; Two Categories Are Surging

Property crime still accounts for 51% of the total rate, but it's been in steady decline since the early 2000s. The five-year drops are steep:
...
The heatmap in Figure 11 shows the shift visually: property violations cool from red to blue over time while violent types hold steady. Figure 12 ranks the largest absolute changes, with theft from vehicles leading all declines at -772 per 100,000.
...
Figures 15 and 16 isolate the individual violations driving these trends. The spotlight in Figure 16 zeroes in on the fastest-growing types.
...
The net effect: violent crime's absolute rate is flat, but its share of total crime grows as the property-crime floor drops away beneath it. The compositional shift is statistically significant (p < 0.05).
...
Here's where the perception gap starts to make sense. Violent crime clears above 50%. Property crime clears below 30% -- meaning 7 in 10 property crimes go unresolved. And these rates haven't budged in two decades.
...
Youth crime has diverged from the adult trend (Figure 19). Youth CSI kept declining after 2014 while adult CSI rose or flattened. Three possible explanations: declining youth population share, diversion programs that keep cases out of the formal system, and generational behavioural shifts. The data confirms the split but can't isolate the cause.
...
The policing model matters too. RCMP detachments and municipal forces show different crime trajectories (Figure 20), and the COVID-19 dip in 2020, followed by a partial rebound, is visible across categories (Figure 21).
...
Two alternative explanations for the overall decline don't hold up. Unfounded rates have stayed stable (Figure 22), so police aren't simply reclassifying more incidents as non-crimes. And the underreporting pattern runs the wrong way (Figure 23): property crime, which is reported at higher rates (~35%), declined more steeply than violent crime (~24% reported). If the decline were just people reporting less, the more-reported category should decline more slowly. The opposite happened.
...
BC police spending grew in both nominal and real (constant 2020 dollar) terms even as crime severity dropped. Salary and benefits growth outpaced operating and capital spending. BC's population grew 10.4% from 5,000,879 in 2018 to 5,519,913 in 2023.
...
The crimes-per-officer workload (Figure S6) tells the story: declining crime volume didn't free up capacity because expanding non-crime demands absorbed it. Police now respond to mental health crises, overdose calls, welfare checks, and social disorder -- work that doesn't generate Criminal Code incidents but consumes patrol hours.
...
If you follow BC crime in the news, Vancouver and Surrey dominate the headlines. They should -- Vancouver logged roughly 48,812 offences and Surrey roughly 41,275 (Figure 24). But per-capita rates reverse the map entirely:
...
Figure 26 is where the interior-coastal divide becomes stark. Interior census metropolitan areas face per-capita rates roughly 2x those of Vancouver and Victoria. And the gap is widening (Figure 27).
...
The divide isn't random. Interior communities share a cluster of compounding risk factors: higher rates of homelessness and visible poverty, acute opioid-crisis exposure, seasonal tourism economies with transient populations, and reliance on RCMP detachments rather than dedicated municipal forces.
...
A composite ranking (60% standardized volume, 40% trend direction) classifies areas as high, moderate, or lower concern, identifying where intervention would have the highest expected return.
...
Now the pieces come together. Crime fell 46% and then another 7.4%. The data is police-reported, sentence-weighted, and statistically significant. And yet 42% of BC residents told the General Social Survey that crime had increased -- a 12-point swing from 30% in 2014 to 42% in 2019, against a falling trend.
...
That perception is factually wrong at the aggregate level. But it persists for five specific, data-traceable reasons. Each has an operational corrective.
...
**3. A 70% unsolved rate destroys trust in the data source.** Property crime clears below 30% (Figures 17-18). This rate hasn't improved in twenty years. When the institution that produces the crime statistics can't resolve 7 in 10 reported cases, the public has a rational basis for discounting its claims. The credibility problem is institutional, not informational.

*Corrective:* Implement closed-loop case reporting. Every filed report receives a status update within 30 days: resolution, referral, or explicit closure with reason.

**4. Provincial averages erase locally high crime.** Chilliwack (11,352/100k) and Kamloops (10,546/100k) face rates 2x Vancouver's 5,438 (Figure 26). For those communities, "crime is falling" doesn't match daily reality. The provincial average combines their experience with lower-crime coastal populations, misleading both groups.

*Corrective:* Publish jurisdiction-level dashboards and allocate resources proportional to per-capita rates.

**5. Visible disorder drives perception but falls outside the CSI.** Open drug use, encampments, and aggressive panhandling are not Criminal Code offences and don't appear in the CSI. In opioid-affected interior communities, they dominate daily experience. The CSI was never designed to capture disorder, and the public was never told this.

*Corrective:* Create a "community disorder index" from police calls for service (wellness checks, trespass, mischief) and publish it alongside the CSI.
...
Reporting rates range from 6% (sexual assault) to 60% (motor vehicle theft). Nationally, 71% of victimizations go unreported (2019 GSS). The statistics the public is asked to trust are built on a foundation where nearly three-quarters of crime never enters the system.
...
**Why now:** Chilliwack (11,352/100k) and Kamloops (10,546/100k) face per-capita rates 2x Vancouver's (5,438/100k). Provincial funding formulas that weight absolute volume over per-capita intensity systematically underfund these communities.
...
**Why now:** The 12-point jump in perceived crime increase (30% to 42%, 2014-2019) occurred while CSI remained below its historical peak. Left unchecked, this gap creates political pressure to respond to perceptions rather than evidence.

**Next steps:** Publish a quarterly plain-language CSI dashboard covering total and violent/non-violent breakdowns with per-capita rates by community. Pair with visible community policing in the 10 highest-rate jurisdictions.
...
2. **Causal drivers of the decline.** The 46% CSI drop from 2003 to 2014 is well-documented but its causes are debated: demographic ageing, rising incarceration through the 2000s, improved security technology, economic conditions, and cultural shifts all contribute.

3. **The effectiveness of specific interventions.** Clearance rates, cost trends, and staffing ratios describe system performance at the aggregate level. They can't attribute outcomes to specific programs, policies, or leadership decisions.
...
**Limitations:** Police-reported data only: the "dark figure" of unreported crime (71% nationally per 2019 GSS) is not captured. COVID-19 distorts 2020-2021. Clearance does not equal conviction. Reporting rates range from 6% (sexual assault) to 60% (motor vehicle theft), creating structural blind spots that no methodology can fully resolve.
